...
from .hybrid_encoder import SoftDropout
...
class MLP(nn.Module):
    """
      1 + h
            ...
            2,4
    """
    def __init__(self, input_dim: int, hidden_dim : int, output_dim : int, num_layers : int, act='relu'):
        super().__init__()
    ...
    def forward(self, x):
        x = x.half() if config.half else x
        for i, layer in enumerate(self.layers):  # 0 nn.linear(1+h,1+h)
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self,
        ...
        self.dropout3 = nn.Dropout(dropout) if not config.soft_dropout else SoftDropout(dropout)
        self.linear2 = nn.Linear(dim_feedforward, d_model)
        self.dropout4 = nn.Dropout(dropout) if not config.soft_dropout else SoftDropout(dropout)
    ...
    def with_pos_embed(self, tensor, pos):
        ...
        pos = pos.half() if config.half else pos
    ...
    def forward(self,
        ...
        # cross attention
        reference_points = reference_points.half() if config.half else reference_points
...
class RTDETRTransformer(nn.Module):
    ...
    def forward(self, feats, targets=None):
        ...
        # feats 拉平 = memory， spatial_shapes 特征大小， level_start_index 就是每个特征图的起始index
        (memory, spatial_shapes, level_start_index) = self._get_encoder_input(feats)

        # prepare denoising training  from DINO,加入扰动
        if self.training and self.num_denoising > 0:
            denoising_class, denoising_bbox_unact, attn_mask, dn_meta = \
                get_contrastive_denoising_training_group(targets, self.num_classes, self.num_queries, self.denoising_class_embed,
                                                         num_denoising=self.num_denoising, label_noise_ratio=self.label_noise_ratio, box_noise_scale=self.box_noise_scale, )
        else:
